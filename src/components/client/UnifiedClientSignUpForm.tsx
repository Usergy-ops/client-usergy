...
import { ConsolidatedOTPVerification } from './ConsolidatedOTPVerification';
...
export function UnifiedClientSignUpForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('=== Starting Unified Client Sign Up ===');
      ...
      if (signUpError) {
        console.error('Unified sign up error:', signUpError);
      ...
      if (data?.error) {
        console.error('Unified sign up response error:', data.error);
        setSignupStatus('error');
      ...
      if (data?.success) {
        console.log('✅ Unified sign up successful:', data);
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            console.log('Transitioning to OTP verification...');
      ...
      console.error('Unified sign up exception:', error);
      setSignupStatus('error');
  ...
  if (currentView === 'otp-verification') {
    return (
      <ConsolidatedOTPVerification
        email={formData.email}
        password={formData.password}
        onSuccess={() => {
          console.log('✅ Unified OTP verification successful');
        }}
